...
int main()
{
    ...
    pair<int, int> nowPose = make_pair(0, 0);
    ...
    while(!q.empty())
    {
        nowPose = q.front();
        q.pop();
        // printf("[nowPose]: [%d, %d]\n", nowPose.first, nowPose.second);

        if (map[nowPose.first][nowPose.second] == -1)
        {
            printf("HaruHaru");
            return 0;
        }
        ...
        {
            int step = map[nowPose.first][nowPose.second];

            //(주의)step 0인 것 있음
            if (step > 0 && nowPose.first + step < n)
                q.push(make_pair(nowPose.first + step, nowPose.second));

            if (step > 0 && nowPose.second + step < n)
                q.push(make_pair(nowPose.first, nowPose.second + step));
        }
    }
